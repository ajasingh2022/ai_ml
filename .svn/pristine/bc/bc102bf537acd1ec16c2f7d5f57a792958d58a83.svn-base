...
public class ConcentratorInstallation extends AbstractWokOrderActivity {
    ...
    private SteppersItem installationPossiblePage;
    ...
    private SteppersItem newConcentratorTechPlan;
    ...
    private SteppersItem simCardinConcentrator;
    ...
    private static String TAG = ConcentratorInstallation.class.getSimpleName();
    ...
    public void addAssuredSteps() {
        try {
            ...
            addToActiveStepIfNotAdded(installationPossiblePage, -1);
    ...
    public void chooseYesNoResponse(String stepid, String responseCode) {
        try {
            fromindex = activeSteps.indexOf(installationPossiblePage) + 1;
            ...
            if (responseCode.equalsIgnoreCase("1")) {
                ...
                if (activeSteps.contains(attachFile))
                    activeSteps.remove(attachFile);
                if (activeSteps.contains(additionalWork))
                    activeSteps.remove(additionalWork);
            }
    ...
    public void newConcentratorTechPlanPageFragmentResponse(String steppersItemIdentifier, String responseCode) {
        try {
            fromindex = activeSteps.indexOf(newConcentratorTechPlan) + 1;
            ...
            if (responseCode.equalsIgnoreCase(NEXT_PAGE_UNIT_VERIFICATION)) {
                ...
                //simCardinConcentrator
                activeSteps.add(fromindex, simCardinConcentrator);
            ...
            writeLog(TAG + "  : newConcentratorTechPlanPageFragmentResponse() ", e);
        }
    ...
    public void registerNewSimCardInfoPageFragmentResponse(String steppersItemIdentifier, String responseCode) {
        try {
            if (steppersItemIdentifier.equalsIgnoreCase("simCardinConcentrator")) {
                fromindex = activeSteps.indexOf(simCardinConcentrator) + 1;
                toindex = activeSteps.indexOf(additionalWork);
                if (fromindex < toindex)
                    activeSteps.subList(fromindex, toindex).clear();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        arguments = new String[]{resources.getString(R.string.can_installation_be_performed_mount_possible), resources.getString(R.string.can_installation_be_performed_mount_possible) + "?"};
        installationPossiblePage = stepFactory.createStep(StepTypeConstants.YESNOSTEP, "ChooseYesNo", arguments);
            WorkorderAdditionalProcessingCallbackListener eventReasonResetProcessor =
                    new WorkOrderEventReasonResetProcessor();
            ((YesNoFragment) installationPossiblePage.getFragment()).setProcessingCallbackListener(eventReasonResetProcessor);
        ...
        arguments = new String[]{resources.getString(R.string.register_new_concentrator_header), "true"};
        newConcentratorTechPlan = stepFactory.createStep(StepTypeConstants.NEWCONCENTRATORTECHPLAN, "newConcentratorTechPlan", arguments);
        ...
        arguments = new String[]{resources.getString(R.string.sim_card_in_conc), String.valueOf(CONSTANTS().WO_UNIT_T__CONCENTRATOR)};
        simCardinConcentrator = stepFactory.createStep(StepTypeConstants.REGISTERNEWSIMCARDINFOPAGE, "simCardinConcentrator", arguments);

        arguments = new String[]{resources.getString(R.string.register_new_cm_header), String.valueOf(CONSTANTS().WO_UNIT_T__CONCENTRATOR)};
        ...
        yesflow.add(newConcentratorTechPlan);
